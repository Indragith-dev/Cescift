...
const ClientSection = ({ clientsRef }) => {
    const [isVisible, setIsVisible] = useState(false);
    const sectionRef = clientsRef || useRef(null);
    ...
    return (_jsxs(SectionWrapper, { id: "clients", ref: sectionRef, children: [_jsxs(SectionTitle, { as: motion.h2, initial: { opacity: 0, y: 30 }, animate: isVisible ? { opacity: 1, y: 0 } : {}, transition: { duration: 0.8, ease: "easeOut" }, children: ["Our Trusted ", _jsx(GradientText, { children: "Clients" })] }), _jsx(ClientsContainer, { as: motion.div, initial: { opacity: 0 }, animate: isVisible ? { opacity: 1 } : {}, transition: { duration: 0.8, delay: 0.3, ease: "easeOut" }, children: _jsx(CarouselWrapper, { children: _jsxs(CarouselTrack, { children: [logos.map((client, index) => (_jsxs(ClientCard, { as: motion.div, whileHover: {
                                    scale: 1.05,
                                    y: -10,
                                    boxShadow: '0 20px 40px rgba(30, 144, 255, 0.2)',
                                    zIndex: 10
                                }, transition: { type: 'spring', stiffness: 300 }, children: [_jsx(ClientImage, { src: client.src, alt: client.name }), _jsx(ClientName, { children: client.name })] }, `first-${client.name}-${index}`))), logos.map((client, index) => (_jsxs(ClientCard, { as: motion.div, whileHover: {
                                    scale: 1.05,
                                    y: -10,
                                    boxShadow: '0 20px 40px rgba(30, 144, 255, 0.2)',
                                    zIndex: 10
                                }, transition: { type: 'spring', stiffness: 300 }, children: [_jsx(ClientImage, { src: client.src, alt: client.name }), _jsx(ClientName, { children: client.name })] }, `second-${client.name}-${index}`)))] }) }) })] }));
};
// Styled Components
